...
- **Create:** `.claude/skills/entry/templates/research.yaml` — YAML template for the research artifact.
...
Expected output: only `research.yaml`.
...
- `research.md` — Exploration, notes, references. Rendered from `templates/research.yaml` on create.
...
Some artifacts are rendered from a YAML template on create. Each such artifact has a matching `templates/<artifact>.yaml` with this shape:
...
1. **Required template.** `templates/<artifact>.yaml` must exist. If missing or malformed, surface a user-facing error and stop.
...
```bash
git add .claude/skills/entry/templates/research.yaml .claude/skills/entry/SKILL.md
